...
class AreaOfAcquisition:
    ...
    def contains_angle(self, psi: float) -> bool:
        ...
        return abs(self.theta - psi) % (2 * np.pi) <= self.phi

    def plot(self, ancour: Position, r_min: float, r_max: float, fillcolor: str, edgecolor: str, alpha: float):
        ...
        cone = mpl.patches.Polygon(points, closed=True, facecolor = fillcolor, edgecolor = edgecolor, alpha = alpha)
