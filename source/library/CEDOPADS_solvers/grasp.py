...
def greedy(problem: CEDOPADSInstance, route: CEDOPADSRoute, sensing_radius: float, rho: float, t_max: float, p: float = 1.0) -> CEDOPADSRoute:
    """Runs a greedy algorithm on the problem instance, with initial routes given"""
    remaining_distance = problem.t_max - problem.compute_length_of_route(route, sensing_radius, rho)
    ...
    angle = np.arcsin((problem.source[1] - problem.sink[1]) / (problem.source[0] - problem.sink[0]))
    tail = State(problem.source, angle) if problem.source[0] < problem.sink else State(problem.source, (angle + np.pi) % (2 * np.pi))
    while True:

        # Compute eligible nodes
        # TODO: We need a way to reduce the number of candidates
        candidates = unvisited_nodes
        # From GRASP implementation for TOP with delunay: unvisited_nodes.intersection(set(tail.adjacent_nodes)) 
        ...
        # TODO: Uncomment if candidates gets updated.
        # if len(candidates) == 0: 
            # candidates = unvisited_nodes

        # Only look at the nodes which allows us to subsequently go to the sink.
        sdr_scores_of_eligible_states = {}
        sdr_scores_of_eligible_states = {node.node_id: (node.score / np.linalg.norm(tail.pos - node.pos)) for node in candidates} # TODO Add distance requirement
        for node in candidates: # Figure out which nodes can be added, without violating the distance constraint.
            pass 
        ...
        if len(sdr_scores_of_eligible_scores) == 0:
            return route 
        
        else:
            if p < 1.0:
                number_of_nodes = int(np.ceil(len(sdr_scores_of_eligible_scores) * (1 - p)))
            else:
                number_of_nodes = 1
            
            rcl = sorted(sdr_scores_of_eligible_scores.keys(), key = lambda node_id: sdr_scores_of_eligible_scores[node_id], reverse=True)[:number_of_nodes] 
            node_id_to_append = np.random.choice(rcl)
            routes.add_node(node_id_to_append)
            remaining_distance -= 

            unvisited_nodes.remove(node_id_to_append) 
...
def grasp(problem: TOPInstance, time_budget: int, p: float, use_centroids: bool = False, maximum_number_of_iterations: int | None = None) -> List[Route]:
