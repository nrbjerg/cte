...
class EuclidianInterceptionRoute(InterceptionRoute):
    ...
    def __init__(self, initial_position: Position, ks: List[int], routes: List[Route], vel: float, terminal_position: Position, delay: float = 0):
        ...
        self.interception_points = [compute_euclid_interception(initial_position, vel, routes[ks[0]], lock_on_time=delay)]
    ...
    def plot(self, c: str = "tab:red", show: bool = True, zorder: int = 5):
    ...
    def get_position(self, time: float) -> Position:
        ...
        for fst, snd in enumerate(self.interception_points[:-1], self.interception_points[1:]):
